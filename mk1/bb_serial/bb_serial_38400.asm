...
bb_putc_10:
		...
		; before first bit
         	; cycles = 7 + 18*18 + 3*4 + 4 + 8 + 4 + 11 = 34 + 8*18 + 4*4 = 194
         	; time = 382 cycles * 135.633 nsec/cycle = 51.81 usec
         	; before remaining bits
         	; cycles = 7 + 39*18 + 3*4 + 4 + 4 + 10 + 4 + 8 + 4 + 11 = = 52 + 7*18 + 4*4 = 194
         	; time = 382 cycles * 135.633 nsec/cycle = 51.81 usec

         	;
         	; delay ~192 clock cycles
         	;
		ld	a,7		; [T=7]
bb_putc_15:
		dec	a		; [T=4]
		...
		; before stop bit 
		; cycles = 7 + 7*18 + 3*4 + 4 + 10 + 2*4 + 4 + 4 + 4 + 11 = 44 + 7*18 + 4*4 + 2*4 = 194
         	; time = 382 cycles * 135.633 nsec/cycle = 51.81 usec
 ...
 bb_putc_25:
 		...
 		nop			; [T=4]
 		; after stop bit
 		; cycles = 7 + 20*18 + 4 = 7 + 10*18 + 1*4 = 191
